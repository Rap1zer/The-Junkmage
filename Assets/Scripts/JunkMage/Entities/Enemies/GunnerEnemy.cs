...
namespace JunkMage.Entities.Enemies
{
    ...
    public class GunnerEnemy : EnemyBase
    {
        ...
        protected override void Start()
        {
            ...
            // Apply random variation to ranges
        ...
        protected override void LookForPlayer()
        {
            ...
            if (dist <= Stats.GetVal(Stat.DetectionRange))
        ...
        protected override void DoChaseBehavior()
        {
            ...
            if (dist <= Stats.GetVal(Stat.AttackRange))
            {
        ...
        protected override void DoAttackBehavior()
        {
            ...
            if (dist < Stats.GetVal(Stat.RetreatRange))
            {
                CurrentState = EnemyState.Fleeing;
                return;
            }

            if (dist > Stats.GetVal(Stat.AttackRange) * Stats.GetVal(Stat.AttackEndMultiplier))
            {
                CurrentState = EnemyState.Chasing;
                return;
            }
        ...
        protected override void DoFleeBehavior()
        {
            ...
            float dist = Vector2.Distance(transform.position, player.transform.position);
            if (dist > Stats.GetVal(Stat.RetreatRange) * Stats.GetVal(Stat.RetreatEndMultiplier))
                CurrentState = EnemyState.Chasing;
        ...
        protected override void Attack()
        {
            ...
            float bulletSize = Stats.GetVal(Stat.BulletSize);
            bullet.transform.localScale = new Vector3(bulletSize, bulletSize, 1f);

            // Initialise bullet with damage information
            ...
            if (b != null) b.Initialise(dmgInfo);

            // Apply force to the bullet
            Rigidbody2D rb = bullet.GetComponent<Rigidbody2D>();
            if (rb != null) rb.linearVelocity = -firePoint.up * Stats.GetVal(Stat.BulletSpeed);
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawWireSphere(transform.position, Stats.GetVal(Stat.AttackRange));

            // Retreat Range (Red) - When the enemy starts fleeing
            Gizmos.color = new Color(1f, 0.2f, 0.2f, 0.75f); // Red
            Gizmos.DrawWireSphere(transform.position, Stats.GetVal(Stat.RetreatRange));
            ...
            Gizmos.DrawWireSphere(transform.position, Stats.GetVal(Stat.DetectionRange));
            ...
            Gizmos.DrawWireSphere(transform.position, Stats.GetVal(Stat.RetreatRange) * Stats.GetVal(Stat.RetreatEndMultiplier));
            ...
            Gizmos.DrawWireSphere(transform.position, Stats.GetVal(Stat.AttackRange) * Stats.GetVal(Stat.AttackEndMultiplier));
        }
    }
}
